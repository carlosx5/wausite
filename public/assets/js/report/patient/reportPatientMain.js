document.addEventListener("DOMContentLoaded", async () => {
    //:Importa módulos
    [{ obj: $m.fPeriod }, { obj: $m.fClinic }] = await Promise.all([
...
async function moduleRender(selectedModule) {
    //:Importa módulo
    if (!$m[selectedModule]) {
        const lastUrl = `reportPatient${selectedModule.charAt(0).toUpperCase() + selectedModule.slice(1)}`;
        const mod = await import(`${jsURL}report/patient/${selectedModule}/${lastUrl}.js?v=${g.refresh}`);
