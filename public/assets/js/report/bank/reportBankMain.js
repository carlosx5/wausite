document.addEventListener("DOMContentLoaded", async () => {
    //:Importa módulos
    [{ obj: $m.fPeriod }, { obj: $m.fBank }] = await Promise.all([
        import(`${jsURL}report/libraries/filter_period.js?v=${g.refresh}`),
        import(`${jsURL}report/libraries/filter_bank.js?v=${g.refresh}`),
    ]);

    //:Local storage
    ls.set("menuTop", "list");
    ls.set("f_dtTarget", "period", "rel_bank");
...
async function moduleRender(selectedModule) {
    //:Importa módulo
    if (!$m[selectedModule]) {
        const lastUrl = `reportBank${selectedModule.charAt(0).toUpperCase() + selectedModule.slice(1)}`;
        const mod = await import(`${jsURL}report/bank/${selectedModule}/${lastUrl}.js?v=${g.refresh}`);
