...
async function moduleRender(selectedModule) {
    //:Importa módulo
    if (!$m[selectedModule]) {
        const lastUrl = `reportOs${selectedModule.charAt(0).toUpperCase() + selectedModule.slice(1)}`;
        const mod = await import(`${jsURL}report/os/${selectedModule}/${lastUrl}.js?v=${g.refresh}`);
