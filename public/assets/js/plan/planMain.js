...
async function moduleRender(selectedModule) {
    ...
    if (!$m[moduleName]) {
        const moduleFile = `plan${moduleName.charAt(0).toUpperCase()}${moduleName.slice(1)}`;
        const importedModule = await import(`${jsURL}plan/${moduleName}/${moduleFile}.js?v=${g.refresh}`);
